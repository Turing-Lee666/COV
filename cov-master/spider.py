...
if __name__ == "__main__":
    # print(sys.argv)  # 获取到所有的参数，至少有一个值就是这个脚本本身所处的绝对路径  ['E:/新冠疫情监控/cov-master/spider.py', '100', '200', '300', 'a']
    l = len(sys.argv)  # 参数列表的长度
    if l == 1:  # 如果参数列表的长度是1，就说明没有传参
        s = """
        请输入参数
        参数说明：  
        up_his  更新历史记录表
        ...
        order = sys.argv[1]  # 0号元素表示该脚本文件的绝对路径  1号元素表示我们传的参数
        if order == "up_his":  # 如果传的是"up_his"这样一个指令
            update_history()
        elif order == "up_det":
            update_details()
        elif order == "up_hot":
